...
function decide({ pathname = '/', query = '', cookie = '', header } = {}) {
  const url = new URL(`http://notes.reef.localhost${pathname}${query}`)
  return authDecision({
...
test('authDecision', async (t) => {
  ...
  await t.test('lets the health probe through unauthenticated', () => {
    assert.equal(decide({ pathname: '/__reef/health' }).action, 'allow')
  })

  await t.test('does not exempt other reserved paths', () => {
    assert.equal(decide({ pathname: '/__reef/sdk.js' }).action, 'deny')
  })
})
