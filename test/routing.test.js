...
test('parseHostname', async (t) => {
  await t.test('extracts the app id', () => {
    assert.equal(parseHostname('notes.colony.localhost:7777'), 'notes')
  })

  await t.test('works without a port', () => {
    assert.equal(parseHostname('notes.colony.localhost'), 'notes')
  })

  await t.test('allows hyphens in the app id', () => {
    assert.equal(parseHostname('feed-reader.colony.localhost:7777'), 'feed-reader')
  })
  ...
  await t.test('rejects the bare gateway host', () => {
    assert.equal(parseHostname('colony.localhost:7777'), null)
  })
  ...
  await t.test('rejects a host that merely ends in the suffix', () => {
    assert.equal(parseHostname('notcolony.localhost'), null)
  })

  await t.test('rejects nested subdomains', () => {
    assert.equal(parseHostname('a.b.colony.localhost'), null)
  })

  await t.test('rejects an app id with a path separator', () => {
    assert.equal(parseHostname('../etc.colony.localhost'), null)
  })
