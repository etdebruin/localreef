# Local Desktop app format

An app is a folder. **The manifest is optional** — Local Desktop infers what it can, and you
only write `desktop.json` to override something it got wrong or to declare a capability.

An app that works in Local Desktop still runs standalone outside it. That's a constraint, not a
...
| `env` | `{}` | Merged over Local Desktop's injected vars (below) |
...
HOST             127.0.0.1
DESKTOP_APP_ID      "notes"
DESKTOP_DATA_DIR    ~/Library/Application Support/Local Desktop/data/notes
DESKTOP             "1"
```

Bind to `HOST`, not `0.0.0.0`. If your framework ignores `PORT` (Vite does), Local Desktop
...
instead and Local Desktop will look for it there:
...
Absent from the manifest means denied. Local Desktop prompts on first use and remembers the
answer.

| Permission | Grants |
|---|---|
| `storage` | `desktop.storage.*` — per-app persisted KV |
| `ai` | `desktop.ai.complete()` — model access through Local Desktop's key, rate-limited |
...
Also zero config. Local Desktop spawns it, adopts the port Vite prints, and proxies WebSocket
...
The gateway owns `/__desktop/*` on every app origin and will shadow anything you put
there:

| Path | Serves |
|---|---|
| `/__desktop/sdk.js` | The SDK bridge |
| `/__desktop/vendor/*` | Local React, etc. — no CDN, works offline |
| `/__desktop/health` | Gateway readiness probe |
