...
You open Local Colony and see a desktop with icons on it. You click one and your app opens
...
Local Colony's premise: **the shell is the address bar.** An app is `notes`, not
...
```
                             ┌───────────────────────────────────────────┐
  iframe                     │  Local Colony gateway   127.0.0.1:PORT   │
  notes.colony.localhost ──▶│                                           │
...
outside Local Colony.

### Why `*.colony.localhost`
...
was: navigate to `?__colony=<token>`, set an `HttpOnly` cookie, redirect to a clean
...
What works: Electron attaches an `x-colony-token` header to every request bound for
`*.colony.localhost` via `webRequest.onBeforeSendHeaders`, scoped by URL filter so
...
`*://*.colony.localhost/*` never matched a WebSocket handshake: the upgrade arrived
...
Apps arrive three ways: generated by the agent (into `~/…/Local Colony/apps/`), drag-dropped
...
Local Colony works on a machine with no Node at all — that's the difference between "it just
...
```html
<script src="/__colony/sdk.js"></script>
```
...
`window.__colony_bridge`. Registered via `session.registerPreloadScript`, it applies to
...
```js
colony.app.id                  // "notes"
colony.app.setTitle(str)       // window chrome
colony.app.setIcon(emoji)

colony.window.close()
colony.window.setSize(w, h)

colony.storage.get(key)        // per-app KV, persisted, capability-gated
colony.storage.set(key, value)

colony.notify(message)         // native notification

colony.ai.complete(prompt)     // ← the sleeper feature
```

`colony.ai` is worth more than it looks. Apps the agent generates can themselves call a
...
`import React from '/__colony/vendor/react'`.
...
long-term reason Local Colony exists rather than being a nicer launcher.
...
```
~/Library/Application Support/Local Colony/
  desktop.db              apps, windows, layout, intents, shared store
  apps/<id>/           agent-generated apps
  data/<id>/           per-app sandboxed data (also COLONY_DATA_DIR for servers)
...
**M2 — The canvas.** Canvas, icons, window frames, drag shield, focus handling, dock,
...
Then: pop-out, `colony.ai`, intents, shared store. (Python arrived early —
