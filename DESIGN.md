...
You open Local Reef and see a desktop with icons on it. You click one and your app opens
...
Local Reef's premise: **the shell is the address bar.** An app is `notes`, not
...
```
                             ┌───────────────────────────────────────────┐
  iframe                     │  Local Reef gateway   127.0.0.1:PORT   │
  notes.reef.localhost ──▶│                                           │
...
was: navigate to `?__reef=<token>`, set an `HttpOnly` cookie, redirect to a clean
...
What works: Electron attaches an `x-reef-token` header to every request bound for
`*.reef.localhost` via `webRequest.onBeforeSendHeaders`, scoped by URL filter so
...
`*://*.reef.localhost/*` never matched a WebSocket handshake: the upgrade arrived
...
Apps arrive three ways: generated by the agent (into `~/…/Local Reef/apps/`), drag-dropped
...
Local Reef works on a machine with no Node at all — that's the difference between "it just
...
```html
<script src="/__reef/sdk.js"></script>
```
...
`window.__reef_bridge`. Registered via `session.registerPreloadScript`, it applies to
...
```js
reef.app.id                  // "notes"
reef.app.setTitle(str)       // window chrome
reef.app.setIcon(emoji)

reef.window.close()
reef.window.setSize(w, h)

reef.storage.get(key)        // per-app KV, persisted, capability-gated
reef.storage.set(key, value)

reef.notify(message)         // native notification

reef.ai.complete(prompt)     // ← the sleeper feature
```

`reef.ai` is worth more than it looks. Apps the agent generates can themselves call a
...
long-term reason Local Reef exists rather than being a nicer launcher.
...
```
~/Library/Application Support/Local Reef/
  desktop.db              apps, windows, layout, intents, shared store
  apps/<id>/           agent-generated apps
  data/<id>/           per-app sandboxed data (also REEF_DATA_DIR for servers)
...
Then: pop-out, `reef.ai`, intents, shared store. (Python arrived early —
